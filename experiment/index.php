...
//Get all experiment
$app->map(['GET', 'POST', 'PUT'],'/get_all', function(Request $request, Response $response){
	$db_name = $request->getParam('name');
	//$table = $request->getParam('table');
	//$fingerprint = $request->getParam('fingerprint');
    $sql = "SELECT * FROM experiment where tool_category='expression' order by experiment_id";
    try{
        $db = new db();
        $db = $db->connect($db_name);
        $stmt = $db->query($sql);
